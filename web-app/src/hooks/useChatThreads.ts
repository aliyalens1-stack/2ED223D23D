/**
 * useChatThreads — canonical thread list polling (web, Sprint B2).
 *
 * Reads `GET /api/chat/v1/threads`. Identical wire contract to the mobile
 * hook; browser bindings only.
 */
...
export function useChatThreads(enabled: boolean = true): UseChatThreadsResult {
  ...
  const fetchPage = useCallback(async (cursor: string | null, append: boolean) => {
    ...
    try {
      ...
      if (!mountedRef.current) return;
      if (e?.response?.status === 401) { setThreads([]); setError(null); }
  ...
  const refresh = useCallback(async () => { setNextCursor(null); await fetchPage(null, false); }, [fetchPage]);
  const loadMore = useCallback(async () => { if (nextCursor) await fetchPage(nextCursor, true); }, [nextCursor, fetchPage]);

  useEffect(() => {
    mountedRef.current = true;
    if (!enabled) { setLoading(false); return () => { mountedRef.current = false; }; }
    void fetchPage(null, false);
    const h = window.setInterval(() => void fetchPage(null, false), POLL_INTERVAL_MS);
    return () => { mountedRef.current = false; window.clearInterval(h); };
  }, [fetchPage, enabled]);

  useEffect(() => {
    if (!enabled) return;
    const onFocus = () => { if (document.visibilityState === 'visible') void fetchPage(null, false); };
